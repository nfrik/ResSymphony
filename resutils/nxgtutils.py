...
def gt2nx(gtG):
    """
        Converts a graph-tool graph to networkx graph.
        """
    # Phase 0: Create a directed or undirected graph-tool Graph
    nxG = nx.Graph(directed=gtG.is_directed())

    # we convert only edges and nodes
    for gtedge in gtG.get_edges():
        nxG.add_edge(int(gtedge[0]),int(gtedge[1]))
...
if __name__ == '__main__':
    ...
    nxG2=gt2nx(gtG)
    for item in nxG2.edges(data=True):
        print(item)


def generate_lattice(n, dim=2, rmp=0., periodic=False,):
    g=gt2nx(lattice([n]*dim,periodic=periodic))
    if rmp > 0:
        edgs=[]
        for e in g.edges():
            edgs.append(e)
